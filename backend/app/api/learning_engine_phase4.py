...
async def get_learning_engine(user: User = Depends(require_pro_user), session: AsyncSession = Depends(get_db)):
...
async def analyze_learning(user: User = Depends(require_pro_user), session: AsyncSession = Depends(get_db)):
